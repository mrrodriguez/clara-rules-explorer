...
;; kondo analyzes .cljc for :cljs too, where `slurp`/`format` do not exist;
;; this file loads only on JVM/bb (both have them), so that finding is
;; suppressed on the one form that needs them.
#_:clj-kondo/ignore
(defn editor-token-resolve-form
  "Builds the editor resolve form (as a string) for CALLER-NS and TOKEN by
   filling the canonical resource template's slots with string literals, so
   arbitrary token text cannot break out of the form. Evaluating the result
   over a repl yields an fq class/var name for resolvable symbols, the `str`
   of keywords, the raw token text for any other readable form (the client
   normalizes those), and nil when the token is unreadable — the editor falls
   back to the raw token then."
  [caller-ns token]
