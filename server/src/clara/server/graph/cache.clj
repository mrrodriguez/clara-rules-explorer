(ns clara.server.graph.cache
  "Analysis and session-snapshot caching for the Clara Rules Explorer server.
   ...
   Wraps a single atom that holds the cached analysis state plus the derived session snapshot. Both
  `clara.server.graph.api` (HTTP handlers) and `clara.server.graph.server` (lifecycle) use this
  namespace; the cache atom itself lives in `clara.server.graph.server` as a `defonce` so
  `clara.server.graph.server/swap-session!` can access it.

   Invalidation is automatic: every access compares the current session and annotations references
  against the cached ones via `identical?`. When either atom is `reset!`, the cache misses and
  rebuilds."
...
(defn- get-state
  "Returns the cached state map, rebuilding when the session or annotations
   ...
   is unavailable)."
  [cache session-atom annotations-atom]
