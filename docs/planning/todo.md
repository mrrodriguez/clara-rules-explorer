1. DONE production summary view when on "fact types" tab and in the "LHS types (input)" expanding a LHS
...
2. DONE production summary In "fact types" tab in the "LHS types (input)" the expand tooltip is "show
...
3. DONE `(defrule x-rule [:not [:my-ns/my-type]] => 1)` does not detect :my-ns/my-type as a fact type
