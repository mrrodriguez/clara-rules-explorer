...
- [x] Step 7: generic boundary chain multi-hop local tracing — `find-local-binding` takes an explicit span, `trace-local-form` threads the current span via `init-form-span`; `usage->span` shared with `expanded-regions`
...
- [x] Docstring fixes: cross-ns references now resolve — `index/…` (in scope) in `analyze.callsite`, fully-qualified `clara.server.tools.graph.analyze.callsite/expanded-regions` in `analyze.index` (no `callsite` alias there)
