...
- [x] Rename `synth` fns per §4 (`->require-clauses`, `->import-clauses`, `->unmapped-default-imports`) + `get-var-ns-name` / `get-var-name`, delegating to leaf (`core-deviations` later removed outright — see Phase 6)
- [x] Parity tests (plan §9.1 data-syntax parity, §9.2 reconstructed-source stability) — new `ns-deps-test` + `analyze/synth_test.clj`, 5 tests / 28 assertions
...
- 2026-09-12: Post-implementation review follow-up. Plan corrected for the final state: `->core-deviations` marked removed (was only ever described as a rename), §7 leaf-requires list fixed to `clojure.java.io`/`clojure.string`/`schema.core`, stale line-number refs dropped, §3 schema-validation wording made accurate (leaf uses `s/defn` test-time validation; `synth` uses `s/validate`), status moved to Implemented. New coverage: `integration_test` asserts `ns-deps` in the `/v1/rulebase-analysis` response (rule + query ns), and `ns-deps-test` gains fallback tests (nil/throwing `base-source-fn`, non-`(ns …)` first form) plus a `.cljc` read-cond `:clj`-branch test. Green: 288 tests, 1829 assertions, 0 failures.
