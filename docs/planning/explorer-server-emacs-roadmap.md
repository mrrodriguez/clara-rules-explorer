...
- [x] Elisp tiered tests — `editor/emacs/test/clara-explorer-test.el` (43 ERT, see
  ...
  - [x] **Gate:** `make -C editor/emacs test-tier1` (CI, stubbed) and `make eldev-prepare && make -C editor/emacs eldev-test` (local, real deps) + `make -C editor/emacs check-elisp` (byte-compile vs stubs) — `--verbose` removed (Eldev 1.11 uses `-v` global)
...
- [x] **Gate:** `make -C editor/emacs test-tier1` + `make -C editor/emacs check-elisp` +
...
| Elisp change  | `make -C editor/emacs test-tier1` (Tier 1 stubbed, CI) + `make -C editor/emacs eldev-prepare && make -C editor/emacs eldev-test` (Tier 2 real deps, local; 0 skipped) + `make -C editor/emacs check-elisp` (automated byte-compile vs stubs) + `M-x eval-buffer` manual nav; `.eldev/` ignored (`make clean` wipes) |
