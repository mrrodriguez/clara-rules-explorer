...
Every navigation pushes the _origin_ onto both jump lists **before** the jump, so `C-o` (`evil-jump-backward`) / `C-i` and `M-.` / `M-*` (`xref-pop-marker-stack`) work regardless of path:
...
_If you see `fallback:` you are on the `var? nil` path_ (non-var production, or tuple/string type whose var has no `:file`/`:line`). If `var?=t` but `C-o` still fails, check `evil-jump-list` (`M-: (evil-jump-list)`) and `*Messages*` for `push` lines — the origin should appear before `cider-find-*`.
