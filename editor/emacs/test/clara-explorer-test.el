...
(ert-deftest bb-root-reads-env-var ()
  (let ((clara-explorer-bb-root nil))
    (cl-letf (((symbol-function 'getenv)
               (lambda (k) (when (equal k "CLARA_RULES_EXPLORER_REGISTRY") "/reg"))))
      (should (equal (clara-explorer--bb-root) "/reg")))))

(ert-deftest bb-root-defcustom-wins ()
  (let ((clara-explorer-bb-root "/custom"))
    (should (equal (clara-explorer--bb-root) "/custom"))))

(ert-deftest prompt-bb-selection-builds-edn ()
  (cl-letf (((symbol-function 'clara-explorer--bb-root) (lambda () "/reg"))
            ((symbol-function 'file-directory-p) (lambda (_d) t))
            ((symbol-function 'read-string) (lambda (_prompt) "loan-app-ruleset")))
    (should (equal (clara-explorer--prompt-bb-selection)
                   "{:root \"/reg\" :units [{:repo \"loan-app-ruleset\"}]}"))))

(ert-deftest bb-selection-caches ()
  (let ((clara-explorer--bb-selection-cache nil))
    (cl-letf (((symbol-function 'clara-explorer--prompt-bb-selection)
               (lambda () "{cached}")))
...
(ert-deftest eval-bb-runs-and-parses ()
...
(ert-deftest navigate-dispatches-to-bb ()
  (let (captured-sel captured-input)
    (cl-letf (((symbol-function 'cider-connected-p) (lambda () t))
              ...
              ((symbol-function 'clara-explorer--eval-bb)
               (lambda (sel input)
